...
class _$PopupDialog {
  ...
  Color get _backgroundColor => backgroundColor ?? Palette.onSurfaceInverted;
          ...
          final _bar = Flushbar(
            titleText: title != null && title!.isNotEmpty
                ? AdaptiveText(
                    ...
                    textColor: _backgroundColor.invertLuminance,
                    textColorDark: _backgroundColor.invertLuminance,
                  )
                : titleWidget,
          ...
          try {
            ...
            if (callbackOnRender == true) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                _future = context?.let((it) => _bar.show(it).then((_) => callback?.call(_)));
              });
            } else {
              callback?.call(null);

              WidgetsBinding.instance.addPostFrameCallback((_) {
                _future = context?.let((it) => _bar.show(it));
              });
            }
